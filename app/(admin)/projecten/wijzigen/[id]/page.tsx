...
export default function ProjectWijzigenPage() {
  ...
  useEffect(() => {
    async function loadProject() {
      ...
      const { data: project, error } = await supabase
        .from("projecten")
        .select(
          "id, naam, beschrijving, opmerkingen, start_datum, eind_datum, locatie_id, locaties(id,naam,type,plaats,adres,extra_checkin,contact_persoon,telefoonnummer,percelen!inner(naam))",
        )
        .eq("id", projectId)
        .single();

      if (error || !project) {
        showToast("Project kon niet worden geladen", "error");
        setLoadingProject(false);
        return;
      }

      setProjectnaam(project.naam ?? "");
      setBeschrijving(project.beschrijving ?? "");
      setOpmerking(project.opmerkingen ?? "");
      setStartDatum(project.start_datum?.slice(0, 10) ?? "");
      setEindDatum(project.eind_datum?.slice(0, 10) ?? "");
      const loc = project.locaties as any;
      if (loc) {
        setSelectedLocatie({
          id: loc.id,
          naam: loc.naam,
          ...
          plaats: loc.plaats,
          adres: loc.adres,
          extra_checkin: loc.extra_checkin,
          contact_persoon: loc.contact_persoon,
          telefoonnummer: loc.telefoonnummer,
          perceel: (loc.percelen as any)?.naam,
        });
      }
      ...
      if (projectVloeren?.length) {
        setSelected((prev) => ({
          ...prev,
          vloerIds: projectVloeren.map((v) => v.kamervloer_id),
        }));
      }
      ...
      const { data: projectBussenData } = await supabase
        .from("project_bussen")
        .select(
          "id, bussen(id,naam,type,kenteken), project_bus_medewerkers(medewerker_id)",
        )
        .eq("project_id", projectId);

      if (projectBussenData?.length) {
        setProjectBussen(
          projectBussenData.map((pb: any) => ({
            bus: pb.bussen,
            medewerkerIds:
              pb.project_bus_medewerkers?.map((m: any) => m.medewerker_id) ??
              [],
          })),
        );
      }
  ...
  useEffect(() => {
    ...
    if (!loadingProject) loadLocatieData();
  }, [selectedLocatie, loadingProject]);
  ...
  async function handleSubmit() {
    ...
    const { error: updateError } = await supabase
      ...
      .eq("id", projectId);
    if (updateError) {
      showToast("Project kon niet worden bijgewerkt", "error");
      return;
    }

    // Delete old vloeren and insert new
    await supabase.from("project_vloeren").delete().eq("project_id", projectId);
    ...
    if (vloerError) {
      showToast("Vloeren konden niet worden opgeslagen", "error");
      return;
    }

    // Delete old bussen and insert new
    const { data: oldBussen } = await supabase
      .from("project_bussen")
      .select("id")
      .eq("project_id", projectId);
    if (oldBussen?.length) {
      const oldBusIds = oldBussen.map((b) => b.id);
      await supabase
        .from("project_bus_medewerkers")
        .delete()
        .in("project_bus_id", oldBusIds);
      await supabase
        .from("project_bussen")
        .delete()
        .eq("project_id", projectId);
    ...
    for (const pb of projectBussen) {
      const { data: projectBus, error: busError } = await supabase
        .from("project_bussen")
        .insert({ project_id: projectId, bus_id: pb.bus.id })
        .select("id")
        .single();
  ...
  const steps = (
    <div className="space-y-4 md:space-y-5">
      <SectionCard
        ...
        subtitle="Pas de naam en beschrijving aan"
      >
      ...
      {step3Done && (
        <SectionCard
          ...
          subtitle="Stel per categorie de standaardmethode in"
        >
          {loadingCat ? (
                                            ...
                                            vloeren.map((vloer) => (
                                              ...
                                              >
                                                ...
                                                <select
                                                  ...
                                                  className={`w-full sm:w-auto text-xs font-medium bg-slate-50 border rounded-lg px-3 py-1.5 outline-none focus:border-p focus:ring-2 focus:ring-p/10 transition-all cursor-pointer ${vloer.overrideMethodeId ? "border-p/30 text-p" : "border-slate-100 text-slate-600"}`}
                                                >
                                                  <option value="">
                                                    Standaard (
