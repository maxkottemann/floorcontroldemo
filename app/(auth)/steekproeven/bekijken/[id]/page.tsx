  ...
  useEffect(() => {
    async function getSteekproef() {
      ...
      const { data: vloerData, error: vloerDataError } = await supabase
        ...
        .select(
          "id, kamervloer_id, goedgekeurd, opmerkingen, kamer_vloeren(vierkante_meter, vloer_types(naam), kamers(naam, verdiepingen(naam, bouwdeel(naam))))",
        )
        .eq("steekproef_id", id);
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className="hidden sm:flex items-center gap-2 px-3 py-1.5 cursor-pointer text-xs font-semibold text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl transition-colors duration-200"
                  >
