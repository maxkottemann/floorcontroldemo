...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-4 md:px-6 py-4 md:py-5">
      <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-2">
        {label}
      </p>
      <p className="text-2xl md:text-3xl font-bold text-p leading-tight">
        {value}
      </p>
      {sub && (
...
function PieChart({
  ...
  const slices = data.map((d, i) => {
    ...
    const hx1 = cx + hole * Math.cos(angle),
      hy1 = cy + hole * Math.sin(angle);
    const hx2 = cx + hole * Math.cos(angle + sweep),
      hy2 = cy + hole * Math.sin(angle + sweep);
...
function VloerRij({ v, onClick }: { v: VloerNode; onClick: () => void }) {
  return (
    ...
    >
      ...
      <p className="text-sm font-semibold text-slate-700 flex-1 group-hover:text-p transition-colors truncate">
        {v.vloertype_naam ?? "—"}
      </p>
      ...
      {v.status && (
        <span
          className={`text-xs font-bold px-2 py-0.5 rounded-full border shrink-0 ${v.status === "afgerond" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : v.status === "bezig" ? "bg-amber-50 text-amber-600 border-amber-100" : "bg-slate-100 text-slate-500 border-slate-200"}`}
        >
          {v.status}
        </span>
      )}
  ...
  return (
    <div>
      ...
      >
        ...
        <ChevronDownIcon
          className={`w-3.5 h-3.5 text-slate-300 transition-transform shrink-0 ${open ? "rotate-180" : ""}`}
        />
      </div>
      {open && (
...
function BouwdeelRij({
  ...
  return (
    <div className="border-b border-slate-100 last:border-0">
      <div
        onClick={() => setOpen((p) => !p)}
        className="flex items-center gap-3 px-4 md:px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors"
      >
        ...
        <ChevronDownIcon
          className={`w-4 h-4 text-slate-300 transition-transform duration-200 shrink-0 ${open ? "rotate-180" : ""}`}
        />
      </div>
      {open && (
        <div className="px-4 md:px-6 pb-4 pt-1 bg-slate-50/50">
  ...
  useEffect(() => {
    async function getTree() {
      ...
      const { data } = await supabase
        ...
        .select(
          `
        kamervloer_id, reinigings_methodes(naam),
        kamer_vloeren(id, vierkante_meter, status, vloer_types(naam),
          kamers(id, naam, verdiepingen(id, naam, bouwdeel(id, naam))))
      `,
        )
        .eq("project_id", id);
      if (!data) return;
      ...
      for (const row of data) {
        ...
        const kamer = kv.kamers,
          verdieping = kamer?.verdiepingen,
          bouwdeel = verdieping?.bouwdeel;
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {projectBussen.map((pb) => (
              <div key={pb.id} className="px-4 md:px-5 py-4">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                {bouwdeelTree.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <BuildingOfficeIcon className="w-8 h-8 text-slate-200 mb-3" />
