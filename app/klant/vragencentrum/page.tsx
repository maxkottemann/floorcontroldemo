        ...
        <main className="flex-1 overflow-auto p-3 md:p-8">
          <div className=" mx-auto space-y-4 md:space-y-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                onClick={() => router.push("/klant/vragencentrum/nieuw")}
                className="flex items-center gap-2 px-4 py-2.5 bg-p text-white text-sm font-bold rounded-xl shadow-sm hover:bg-p/90 transition-colors cursor-pointer shrink-0"
              >
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center justify-center py-16 text-center px-4">
                <div className="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center mb-4">
                  <ChatBubbleLeftRightIcon className="w-7 h-7 text-slate-300" />
                </div>
                <p className="text-sm font-semibold text-slate-400">
                  {zoek ? "Geen gesprekken gevonden" : "Nog geen berichten"}
                </p>
                <p className="text-xs text-slate-300 mt-1">
                  {zoek
                    ? "Probeer een andere zoekterm"
                    : "Start een nieuw gesprek met Duofort"}
                </p>
                {!zoek && (
                  <button
                    onClick={() => router.push("/klant/vragencentrum/nieuw")}
                    className="mt-4 flex items-center gap-2 px-4 py-2 bg-p text-white text-sm font-bold rounded-xl hover:bg-p/90 transition-colors cursor-pointer"
                  >
                    <PlusIcon className="w-4 h-4" />
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                {filtered.map((thread, i) => (
                  <div
                    key={thread.id}
                    onClick={() =>
                      router.push(`/klant/vragencentrum/${thread.id}`)
                    }
